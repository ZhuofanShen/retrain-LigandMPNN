...
class Protein2LigandLayer(torch.nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30):
    ...
    def forward(self, nn_idx, Y_scale, Y_nodes, h_E_context, h_V, mask_V=None, mask_attend=None):
        ...
        h_message = self.W3(self.act(self.W2(self.act(self.W1(h_EV))))) # [B,L,M,C]
        if mask_attend is not None:
            h_message = mask_attend.unsqueeze(-1) * h_message
        ...
        dh = self.dense(h_Y)
        h_Y = self.norm2(h_Y + self.dropout2(dh))
        if mask_V is not None:
            h_Y = mask_V.unsqueeze(-1).unsqueeze(-1) * h_Y

        #  central residue nodes, neighborhood ligand nodes --> update protein-ligand graph edges
        h_V_expand = h_V.unsqueeze(-2).expand(-1, -1, Y_nodes.size(-2), -1)
        h_EV = torch.cat([Y_nodes, h_E_context, h_V_expand], -1)
        h_message = self.W13(self.act(self.W12(self.act(self.W11(h_EV))))) # [B,L,M,C]
        h_E_context = self.norm3(h_E_context + self.dropout3(h_message))
        return h_Y, h_E_context
